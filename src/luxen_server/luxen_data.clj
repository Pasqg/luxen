...
(defn create-db [mysql-host mysql-port mysql-user mysql-password mysql-database]
...
(defn get-issue
  ...
  ([db issue-id]
   (let [split (clojure.string/split issue-id #"-")]
     (get-issue db (first split) (second split))))
  )

(defn create-issue [db project-id title description]
  (try
...
(defn set-issue-status [db project-id id status]
  (try
...
(defn create-first-issue-for-project [db project-id]
